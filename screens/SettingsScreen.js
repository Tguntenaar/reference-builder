...
const path = 'avatars'
const localFile = 'file:///Users/thomasguntenaar/Library/Developer/CoreSimulator/Devices/9643F18B-C63C-462F-90AF-0EC5C1678899/data/Containers/Data/Application/8216810A-FB0F-4300-86A3-1788971755E3/Library/Caches/ExponentExperienceData/%2540thomasguntenaar%252FreferenceBuilder/ImagePicker/652C26FE-0CFE-4810-A3DE-F45EA6146D01.jpg';
...
function SettingsScreen({ navigation }) {
  ...
  const getID = async () => {
    // get user
    let { attributes } = await Auth.currentAuthenticatedUser().catch(err => console.log(err));
    setId(attributes.sub)
    return new Promise.resolve(id)
  }
  const getAvatarFromStorage = async () => {
    // get photo
    let url = await Storage.get(`${path}/${team.name}${team.id}/avatar${id}.jpeg`)
  ...
  const getInfo = async () => {
    const userData = await API.graphql(
      graphqlOperation(getUser, 
        { id: "b403da70-bea8-4e54-9cff-6a68e9d07f4d"}))
    .catch((error) => console.log(`ERROR: Can't getUSER \n ${error.message}`));

    const { data: { getUser: { name, jobTitle, teams: { items: [ {team} ] }}}} = userData;
    setTeam(team)
    setUsername(name)
